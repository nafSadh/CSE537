class Question1_Solver:
  def __init__(self, threshold=1):
    self.learn('train.data', threshold)
    return
  ...
  def learn(self, train_data, threshold):
...
class DTNode:
  def __init__(self, level, entropy, population, label=None, decisionAttribute=None,
               domain=set(), children={}):
    self.label = label
    self.level = level
  ...
  def mostLikelyChild(self):
    best, minE = None, 2.0
    for v in self.children:
      e = self.children[v].entropy
      if best is None or minE < e:
        best, minE = v, e
    return best

  def __str__(self):
    if self.label is not None:
      return "(" + self.label + "|" + str(round(self.entropy, 3)) + "|" + str(self.population) + ")"

    childrenStr = ""
    for child in self.children:
      childrenStr += '\n' + str(len(self.level)) + self.level + '' + str(child) + ":" + str(self.children[child])

    return "(" + str(self.decisionAttribute) + ":" + str(round(self.entropy, 3)) + "|" + str(
      self.population) + "|" + str(self.domain) + "|" + childrenStr + ")"
...
def ID3(examples, attributes, values, domain, level, threshold=1):
  ...
  if allSame:
    return DTNode(level, entropy, population, label, None, set())
  ...
  if attributes is None or len(attributes) < 1 or population < threshold:
    return DTNode(level, entropy, population, majority, None, set())
  ...
  for v in values[bestAttrib]:
    if len(subsets[v]) < 1:
      children[v] = DTNode(level, entropy, population, majority)
    else:
      children[v] = ID3(subsets[v], remainingAttributes, values, domain,
                        level + '-', threshold)

  return DTNode(level, entropy, population, None, bestAttrib, domain, children)
